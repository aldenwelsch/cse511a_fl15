...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    actions = {}
    positions = []
    #Find ghost's most likely positions
    for ghost in livingGhostPositionDistributions:
      max_val = max(ghost.itervalues())
      ghost_positions = [k for k, v in ghost.iteritems() if v == max_val]
    ...
    for action in legal:
      ...
      for position in positions:
        sucessorDistance = self.distancer.getDistance(position,successorPosition)
        actions.update({action:sucessorDistance})

    #Pick a best action at random
    min_action = min(actions.itervalues())
    buster_actions = [k for k, v in actions.iteritems() if v == min_action]
    random_best_action = buster_actions[int(random.uniform(0,int(len(buster_actions))))]
