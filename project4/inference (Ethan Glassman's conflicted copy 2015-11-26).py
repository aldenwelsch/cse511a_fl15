...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    "*** YOUR CODE HERE ***"
    # This works now!
    # If the state space is too large, don't try to make a list of all ghost
    # location combinations. For state space size 100 and # of ghosts = 4 we
    # have 100^4 = 100,000,000 tuples. Anything more might start to overflow.
    if len(self.legalPositions) < 100:
      #build a list of all possible ghost position combinations, where each of the ghosts can be in any of the possible positions in the boards.

      allPossible = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
      count = len(allPossible)
      ...
      random.shuffle(allPossible)
      self.particles += allPossible[0:numParts]

    else: # This is the case where the state space is too large to compute all the combos
      self.particles = []
      count = 0
      numParts = self.numParticles
      while count < numParts:
        ...
        for i in range(0, self.numGhosts):
          temp = temp + (random.choice(self.legalPositions), )
        #print temp
        #if not temp in self.particles:
        self.particles += [temp]
        count += 1
  ...
  def observeState(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
	# This works now!
    # Find the indices of each ghost that's in jail
    jailedGhosts = []
    for i in range(self.numGhosts):
        if noisyDistances[i] == None:
            jailedGhosts.append(i)

    allWeights = util.Counter()
    # Create the weight for each particle
    for particle in self.particles:
      # Make sure the particle takes jailed ghosts into account
      for ghostIndex in jailedGhosts:
        part = list(particle)
        part[ghostIndex] = self.getJailPosition(ghostIndex)
        particle = tuple(part)
        #p = self.getParticleWithGhostInJail(p, ghostIndex)
      # Create the weight for the particle:
      weight = 1
      # Check the truedistance of a particle with the emission model
      for i in range(self.numGhosts):
        # we know that our ghost is in jail, so there's no probability factor
        if i not in jailedGhosts:
          model = emissionModels[i]
          ghostLocation = particle[i]
          trueDist = util.manhattanDistance(ghostLocation, pacmanPosition)
          weight *= model[trueDist]

      # Add this probability to the weights counter
      allWeights[particle] += weight

    # Set the beliefs
    self.beliefs = allWeights

    # Resample the particles if there are none left:
    if allWeights.totalCount() == 0:
      self.initializeParticles()
    else:
      self.beliefs.normalize()
      for i in range(len(self.particles)):
        newLocation = util.sample(self.beliefs)
